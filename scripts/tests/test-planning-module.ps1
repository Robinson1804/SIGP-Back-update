...
$adminToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOjEwLCJlbWFpbCI6ImFkbWluLXBsYW5uaW5nQGluZWkuZ29iLnBlIiwidXNlcm5hbWUiOiJhZG1pbnBsYW5uaW5nIiwicm9sIjoiQURNSU4iLCJpYXQiOjE3NjU3NTQxOTIsImV4cCI6MTc2NTg0MDU5Mn0.15VoxzIDLIUs3TQZUHNVzBCcZtp3vBvkaSgTKvjLlK8"
$pmoToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOjExLCJlbWFpbCI6InBtby1wbGFubmluZ0BpbmVpLmdvYi5wZSIsInVzZXJuYW1lIjoicG1vcGxhbm5pbmciLCJyb2wiOiJQTU8iLCJpYXQiOjE3NjU3NTQxOTcsImV4cCI6MTc2NTg0MDU5N30.1Qa8LAtK7Zt3g-1EoI7F8XiHFnt4JtxgVMBY1NlXCrA"
$devToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOjEyLCJlbWFpbCI6ImRldi1wbGFubmluZ0BpbmVpLmdvYi5wZSIsInVzZXJuYW1lIjoiZGV2cGxhbm5pbmciLCJyb2wiOiJERVNBUlJPTExBRE9SIiwiaWF0IjoxNzY1NzU0MjA0LCJleHAiOjE3NjU4NDA2MDR9.Yzxr41b4ngSXijGkjQUNSWpEJjkTxed8QO_fyeyxkEE"
...
if ($ids['pgdId']) {
    Test-Api -TestName "7.1.2: Verificar relación PGD → OEI" `
        -Method GET -Endpoint "/pgd/$($ids['pgdId'])/oei" -Token $adminToken `
        -ExpectedStatus @(200)
}

if ($ids['pgdId']) {
    Test-Api -TestName "7.1.3: Verificar relación PGD → OGD" `
        -Method GET -Endpoint "/pgd/$($ids['pgdId'])/ogd" -Token $adminToken `
        -ExpectedStatus @(200)
}

if ($ids['ogdId']) {
    Test-Api -TestName "7.1.4: Verificar relación OGD → OEGD" `
        -Method GET -Endpoint "/ogd/$($ids['ogdId'])/oegd" -Token $adminToken `
        -ExpectedStatus @(200)
}

if ($ids['oegdId']) {
    Test-Api -TestName "7.1.5: Verificar relación OEGD → AE" `
        -Method GET -Endpoint "/oegd/$($ids['oegdId'])/acciones-estrategicas" -Token $adminToken `
        -ExpectedStatus @(200)
}
